...
static uintptr slide_of_executable(const matype_macho_header* program_header) {
    const u32 command_count = program_header->command_count;
...
uintptr mdlk_bootstrap_self(const matype_macho_header *program_header, i32 argc, const char* argv[], intptr slide, const matype_macho_header *self_header) 
{
